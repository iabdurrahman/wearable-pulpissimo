...
void OLED_Init(void)
{
    ...
    OLED_WriteCommand(0x02); // Page Addressing Mode (for SH1106 compatibility)
...
void OLED_Update(void)
{
    ...
    for (uint8_t page = 0; page < 8; page++) {
        ...
        OLED_WriteCommand(0x02);        // Lower column address (SH1106 offset)
        OLED_WriteCommand(0x10);        // Higher column address
                   ...
                   bool color)
{
    ...
    /* Convert uppercase to lowercase since font only supports lowercase */
    if (search_c >= 'A' && search_c <= 'Z')
    {
        search_c += 32;
    }

    /* Search character in font table */
    for (uint8_t i = 0; i < Font5x7_Count; i++)
    {
        if (Font5x7_CharMap[i] == search_c)
        {
            index = i;
            break;
        }
    ...
    for (uint8_t col = 0; col < FONT_WIDTH; col++)
    {
        ...
        for (uint8_t row = 0; row < FONT_HEIGHT; row++)
        {
            ...
            {
                OLED_DrawPixel(x + col, y + row, !color); // Clear background
            }
        }
    }
}
                     ...
                     bool color)
{
    ...
    while (*str)
    {
        ...
        current_x += FONT_WIDTH + 1; // Move to the next character position
        str++;
        ...
        if (current_x + FONT_WIDTH > OLED_WIDTH)
        {
            current_x = 0;
            y += FONT_HEIGHT + 1;
            if (y + FONT_HEIGHT > OLED_HEIGHT) break; // Out of bounds
        }
    }
}
...
static void OLED_WriteCommand(uint8_t cmd)
{
    ...
    unsigned char buf[2];
    buf[0] = 0x00;  // Co=0, D/C#=0 → control byte untuk command
    buf[1] = cmd;
                           ...
                           uint16_t length)
{
    ...
    while (offset < length)
    {
        ...
        buf[0] = 0x40; // Co=0, D/C#=1 → control byte untuk data
